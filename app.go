...
func (a *App) startup(ctx context.Context) {
	...
	backend.SetSpotifyAuthHandlers(
		func(target string) { runtime.BrowserOpenURL(ctx, target) },
		func() {
			runtime.WindowShow(ctx)
			runtime.WindowUnminimise(ctx)
...
// SpotifyAuthStatus reports whether a Spotify account is connected.
...
func (a *App) spotifyClientIDFromSettings() (string, error) {
	settings, err := a.LoadSettings()
	if err != nil {
		return "", err
	}
	clientID, _ := settings["spotifyClientID"].(string)
	clientID = strings.TrimSpace(clientID)
	if clientID == "" {
		return "", fmt.Errorf("add your Spotify Client ID in Settings first")
	}
	return clientID, nil
}

// ConnectSpotifyAccount kicks off (or silently refreshes) a Spotify login.
// It blocks until the loopback browser flow resolves, is denied, or times out.
func (a *App) ConnectSpotifyAccount() (SpotifyAuthStatus, error) {
	clientID, err := a.spotifyClientIDFromSettings()
	if err != nil {
		return SpotifyAuthStatus{}, err
	}
	record, err := backend.EnsureSpotifyAuth(clientID)
	if err != nil {
		return SpotifyAuthStatus{}, err
	}
...
// GetSpotifyAuthStatus reports the persisted connection state without
// triggering any network activity or browser flow.
func (a *App) GetSpotifyAuthStatus() (SpotifyAuthStatus, error) {
	info, err := backend.LoadSpotifyConnectionInfo()
	if err != nil {
		return SpotifyAuthStatus{}, err
	}
	return SpotifyAuthStatus{
		...
		SpotifyUserID: info.SpotifyUserID,
	}, nil
}

func (a *App) DisconnectSpotifyAccount() error {
	return backend.ClearSpotifyAuth()
}

func (a *App) GetSpotifyRedirectURI() string {
	return backend.SpotifyOAuthRedirectURI()
}

func (a *App) spotifySeparatorFromSettings() string {
	settings, err := a.LoadSettings()
	separator := ", "
	if err == nil && settings != nil {
		if sep, ok := settings["separator"].(string); ok {
...
func (a *App) GetSpotifyLibraryPlaylists() (string, error) {
	...
	client, err := backend.NewSpotifyLibraryClient(clientID)
	if err != nil {
		return "", err
	}
	playlists, err := client.ListMyPlaylists(ctx)
	if err != nil {
		return "", err
	}
...
func (a *App) GetSpotifyLibraryAlbums() (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()
	...
	albums, err := client.ListSavedAlbums(ctx)
	if err != nil {
		return "", err
	}
...
func (a *App) GetSpotifyPlaylistTracks(playlistID string) (string, error) {
	if playlistID == "" {
	...
	payload, err := client.GetPlaylistTracks(ctx, playlistID, a.spotifySeparatorFromSettings())
	if err != nil {
		return "", err
	}
...
func (a *App) GetSpotifyAlbumTracks(albumID string) (string, error) {
	if albumID == "" {
	...
	payload, err := client.GetAlbumTracks(ctx, albumID, a.spotifySeparatorFromSettings())
	if err != nil {
		return "", err
	}
	data, err := json.Marshal(payload)
...
func (a *App) GetSpotifyLikedSongs() (string, error) {
	...
	payload, err := client.GetLikedSongs(ctx, a.spotifySeparatorFromSettings(), func(fetched, total int) {
		runtime.EventsEmit(a.ctx, "spotify-library-progress", map[string]int{"fetched": fetched, "total": total})
	})
	if err != nil {
		return "", err
	}
